(() => {
  ...
  (() => {
    ...
    const u = function (e) {
      const t = +!e;
      const i = e === 0;
      const s = this.view.getValue()[0] > this.view.getValue()[1];
      const a = this.view.getValue()[1] < this.view.getValue()[0];
      return (s && i) || (a && !i) ? t : e;
    };
    const d = function (i) {
      ...
      const a = s => {
        !(function (i, s, a) {
          ...
          const { DOMroot: h } = s.view;
          const c = s.view.getSize();
          const u = l === t.horizontal ? i.pageX : i.pageY;
          const d = s.view.calculateCursorOffset(u, l, h, c);
          const o = n ? s.validateCollision(a) : a;
      ...
      clickTrack() {
        this.view.DOMroot.addEventListener('pointerdown', i =>
          (function (i, s) {
            ...
            const { DOMroot: n } = s.view;
            const r = s.view.getSize();
            const h = a === t.horizontal ? i.pageX : i.pageY;
            const c = s.view.calculateCursorOffset(h, a, n, r);
    ...
    const f = function (e, i, s, a) {
      ...
      const n = l - 1;
      const r = p(n, [3, 5, 7, 11]);
      let h = Math.max(Math.floor(n / r), 1);
      h = h < 15 ? Math.min(h, r) : h;
      const c = Math.ceil(l / h);
      const u = Math.abs(i - e);
      const d = Math.floor(u / s) > c ? Math.round(Math.round(u / s) / (c - 1)) * s : s;
      return {
    ...
    const j = function () {
      ...
      const i = e === t.horizontal ? t.vertical : t.horizontal;
    ...
    const Y = class extends r {
      ...
      updateThumb(t, i) {
        ...
        const a = this.calculateStepPercent();
        const l = Math.round(s / a) * a;
      ...
      updateThumbAfterTrackClick(e) {
        ...
        const { fillOffset: a, fillSize: l } = this.getFillState();
        let n = 0;
        e > l / 2 + a && (n = 1),
    ...
    function ie({
      ...
      const p = e != null ? e : c.min;
      const f = t != null ? t : c.max;
      const b = s != null ? s : c.decimalPlaces;
      const v = a != null ? a : c.step;
      const w = l != null ? l : c.isRange;
      const g = n != null ? n : c.direction;
      const S = r != null ? r : c.hasFill;
      const V = h != null ? h : c.hasTips;
      const P = u != null ? u : c.hasScale;
      const T = d != null ? d : c.isDecimal;
      const O = o != null ? o : c.panel;
      const F = m != null ? m : c.onChange;
      let M = i != null ? i : c.value;
      return (
        Array.isArray(M) || (M = [M]),
        l && M.length === 1 && M.push(M[0]),
        {
          min: p,
          max: f,
          value: M,
        ...
        super(),
          ...
          (this.presenter = new (class {
            ...
            setSubViewsState() {
              ...
              const i = this.model.getValue();
              const s = this.model.getFillState();
              return (
                i.forEach((t, i) => {
